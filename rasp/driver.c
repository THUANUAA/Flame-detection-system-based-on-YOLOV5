...
static void parse_dht11_datas(struct timer_list *t);
...
static irqreturn_t dht11_isr(int irq, void *dev_id)
{
	...
	if (g_dht11_irq_cnt == 84)
	{
		...
		parse_dht11_datas(&gpio_desc->key_timer);
	}
...
static int __init dht11_init(void)
{
	...
	{		
		...
		//timer_setup(&gpios[i].key_timer, key_timer_expire, (unsigned long)&gpios[i]);
